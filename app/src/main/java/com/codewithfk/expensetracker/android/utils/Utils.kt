...
object Utils {
    ...
    fun formatDateToHumanReadableForm(dateInMillis: Long): String {
        // Use lowercase yyyy for year (calendar year), not week-year
    ...
    fun formatCurrency(amount: Double, locale: Locale = Locale("vi", "VN")): String {
        // VND typically has no fractional digits
        val currencyFormatter = NumberFormat.getCurrencyInstance(locale)
        currencyFormatter.maximumFractionDigits = 0
        try {
            currencyFormatter.currency = Currency.getInstance("VND")
        } catch (e: Exception) {
            // fallback: ignore
        }
